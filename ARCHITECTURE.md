...
| `AuthFailed` | Failed unlock (wrong master password on `open`/`unlock`; wrong user password or MFA code on `unlock_vault_as_user`) | `-` |
...
| **Create** | `Vault::create` / `create_v3` acquire the lock for the whole session (released on write failure) |
...
| `minMasterPasswordLen` | Master/user password validation on **all** password-set paths: `Vault::create`, `create_vault_v3`, `add_vault_user`, `change_user_password` (`policy::effective_min_master_password_len`). Migration re-wrap remains exempt |
