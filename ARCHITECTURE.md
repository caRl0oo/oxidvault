...
| **Memory (m)** | 128 MiB (131072 KiB) | B2B-grade brute-force protection for **new** vaults and password-set paths |
| **Iterations (t)** | 3 | OWASP recommendation for Argon2id |
| **Parallelism (p)** | 4 | Balanced for desktop hardware |
| **On-disk minimum** | m ≥ 16 MiB, t ≥ 2, p ≥ 1 | `KdfParams::enforce_minimums()` on per-user unlock — rejects KDF downgrade tampering |
| **Admin override** | `kdfMemoryMib` 64–1024 MiB | `policy::effective_kdf_params_for_new_vaults()` — applies on `create_vault_v3`, `add_vault_user`, `change_user_password` when admin GPO sets the field |

**Implementation status:** ✅ Implemented in `crates/vault-core/src/crypto.rs` (`KDF_MEMORY_KIB`, `MasterKey::derive_from_password`, `KdfParams::enforce_minimums`) · `crates/vault-core/src/policy/mod.rs` (`effective_kdf_params_for_new_vaults`).
...
> Applies when **setting** a user password (`create_vault_v3`, `add_vault_user`, `change_user_password`). When unlocking existing vaults, the original password length is respected.
...
**Legacy format:** `VaultError::UnsupportedLegacyFormat { version }` — stable IPC code `unsupported_legacy_format` (see §7).
...
| **AAD** | v4: full plaintext header (magic → end of `users_json`) bound as AES-GCM AAD |
...
|---|---|
| `encrypt(key, plaintext)` | Fresh 12-byte nonce + ciphertext (empty AAD — non-vault use) |
| `encrypt_with_aad(key, plaintext, aad)` | v4 vault payload encryption — header bytes as AAD |
...
| **Incoming passwords (K4)** | `Zeroizing<String>` in `create_vault_v3`, `unlock_vault_as_user`, `change_user_password` |
| **session_kek (v4)** | `Zeroizing<[u8; 32]>` on `Vault` — KEK of current user; RAM only during session; set to `None` on `lock()` (ZeroizeOnDrop) |
...
| No plaintext in logs | No `Debug` output for sensitive structs (`MasterKey` without `Debug`; `SecretPayload` with manual redacting `Debug` — secret fields print `<redacted>`) |
...
| **Clear on lock** | `SecureClipboard::clear_pending()` in `perform_lock()` — locking within the 30 s window clears the clipboard **immediately** instead of leaving the secret behind |
...
Frontend: notifyBackendSecureCopy() → Toast-Countdown
        │
        ▼
Bei lock_vault: clear_pending() — Clipboard sofort geleert (falls unverändert), Timer invalidiert
```
...
│   ├── extension.id.example        ← Dev unpacked extension ID (copy → extension.id)
│   ├── chrome-store-extension.id   ← ★ Single source: Chrome Web Store extension ID
│   └── host/
│       └── com.oxidvault.app.json.example  ← Template; generated manifest is gitignored
│
├── scripts/
│   ├── register_native_host.ps1   ← Registry + host manifest (Chrome/Edge)
│   ├── native-messaging-host.ps1  ← Store ID + allowed_origins helpers
│   ├── tauri-dev.ps1
│   ├── tauri-build.ps1
│   ├── generate-icons.mjs
...
1. User selects `.oxid` file → `open_vault(path)` attaches the vault **locked** (v4 header metadata only; no password on this command).
2. Frontend receives `UnlockVaultResponse { unlocked: false, isMultiUser: true }` — multi-user login form with user selection.
3. User picks username + password (+ optional TOTP) → `unlock_vault_as_user`.
4. Tauri command forwards to `vault-core::Vault::unlock_as_user` (`Zeroizing<String>` for password and MFA code).
5. **Atomic authentication:** Decryption only in ephemeral stack memory (`EphemeralDecrypt` with `Drop` zeroizing). Password check → optional MFA check → only then commit `VaultHandle`.
6. **Without MFA:** `UnlockVaultResponse { unlocked: true, currentUsername }` — DEK/entries applied to `Vault`.
7. **With MFA, no code:** `AuthError::MfaRequired` → `UnlockVaultResponse { mfaRequired: true }` — **no** key material on `Vault`.
8. **With MFA + code:** repeat `unlock_vault_as_user` with `password` + `mfaCode` — atomic full unlock in one step.
9. `unlock_vault` (no parameters) re-attaches the current path and returns the same **multi-user pending** response when the vault is locked (e.g. after `lock_vault` or bootstrap).
10. On error (`InvalidUserPassword`, `InvalidMfaCode`, `UnsupportedLegacyFormat`): immediate abort, ephemeral memory zeroized.

**Per-user unlock (`unlock_vault_as_user`):**
...
4. Enable/disable MFA: TOTP secret KEK-encrypted in plaintext header; **payload re-encryption** required (header bound via AAD).

**Limitation (documented honestly):** TOTP gates the **application unlock flow** only — it is **not** a second cryptographic factor for the vault file. An attacker with a copy of the `.oxid` file can brute-force a user password offline and bypass TOTP entirely. A **strong user password** (length, entropy, blocklist) and high KDF memory remain the actual file-level defense.
...
| `open_vault` | `path` | `UnlockVaultResponse` | Attach locked v4 vault (`Vault::attach_locked`); returns `multi_user_pending` — **no password on this command** | ✅ |
| `unlock_vault` | — | `UnlockVaultResponse` | Re-attach current path locked; returns `multi_user_pending` for user-login UI | ✅ |
...
| `reencrypt_vault` | `current_password`, `new_password` | `()` | Per-user password rotation — delegates to `Vault::change_own_password` — **vault lock guard** | ✅ |
...
| `create_vault_v3` | `path`, `vault_name`, `admin_username`, `admin_password` | `VaultInfo` | New **v4** multi-user vault with first admin user; passwords → `Zeroizing<String>` | ✅ |
| `attach_vault_path` | `path` | `VaultInfo` | Attach vault file (locked) — user list from v4 header | ✅ |
| `unlock_vault_as_user` | `username`, `password`, `mfa_code?` | `UnlockVaultResponse` | Unlock as specific user (password/code → `Zeroizing<String>`) | ✅ |
| `list_vault_users` | — | `VaultUserPublic[]` | All users (no secrets) — works when v4 vault locked | ✅ |
...
| `change_user_password` | `current_password`, `new_password` | `()` | Change own password (v4 KEK rewrap + payload re-encrypt) — **vault lock guard** | ✅ |
...
| `is_multi_user` | `boolean` | `true` = v4 multi-user format (always for supported `.oxid` files) |
...
| `isMultiUser` | `boolean` | `true` after `open_vault` / `unlock_vault` — frontend shows user-login form |
| `currentUsername` | `string \| null` | Logged-in user after successful `unlock_vault_as_user` |
...
| `createVaultV3(path, vaultName, adminUsername, adminPassword)` | `create_vault_v3` |
| `attachVaultPath(path)` | `attach_vault_path` |
| `unlockVaultAsUser(username, password, mfaCode?)` | `unlock_vault_as_user` → `UnlockVaultResponse` |
| `openVault(path)` | `open_vault` → `UnlockVaultResponse` (locked attach, multi-user pending) |
| `unlockVault()` | `unlock_vault` → `UnlockVaultResponse` (multi-user pending) |
...
> **Status:** ✅ Implemented in `crates/vault-core/src/format.rs` — **format version 4 only** (read and write).  
> **Legacy:** v1–v3 removed in this version; migrate legacy `.oxid` files once with **OxidVault ≤ 2.5.1** from [GitHub Releases](https://github.com/caRl0oo/oxidvault/releases).
...
**Key model:** A random **shared DEK** (32 bytes) encrypts the payload. Each user derives a **KEK** from their own password and wraps the same DEK in their `VaultUser` entry.
...
**v4 downgrade guard:** Encrypted payload JSON includes `"format_version": 4`. After decrypt, must match header version; payload `format_version` > header → `VaultError::FormatDowngrade`.

**Legacy rejection:** `parse_header` / `read_multi_user_vault_file` return `VaultError::UnsupportedLegacyFormat { version }` when `format_version` ≠ 4. Stable IPC substring: `unsupported_legacy_format` → `errors.unsupportedLegacyFormat` in `src/lib/errors.ts`.

**Tampering UX:** GCM cannot distinguish wrong password from tampered header/payload — both return `VaultError::InvalidUserPassword` / `InvalidPassword` (no oracle). Downgrade attempts with mismatched inner `format_version` return `FormatDowngrade`.

| Property | Value |
|---|---|
| **Extension** | `.oxid` |
| **Magic bytes** | `0x4F 0x58 0x49 0x44` (`"OXID"`) |
| **Supported version** | **4** only |
| **Integrity** | GCM auth tag per body block; header bound to payload via AAD |
| **Parser limits** | Header length fields are bounded before allocation: `users_json` ≤ 4 MiB, wrapped-DEK ciphertext ≤ 4 KiB (`InvalidFormat` on tampered lengths — DoS guard) |
...
| **Error mapping** | `src/lib/errors.ts` — Rust error substrings → `errors.*` keys (incl. `unsupported_legacy_format` → `errors.unsupportedLegacyFormat`) |
...
2. Enter admin username + password (+ optional vault name)
3. Save dialog: choose `.oxid` file (`pickVaultSavePath`)
4. Backend (`create_vault_v3`):
   - Generate per-user salt + Argon2id (128 MiB default, or admin `kdfMemoryMib`) → user KEK
   - Random shared DEK → AES-256-GCM payload with header AAD → write v4 file
...
3. Backend (`open_vault`): attach locked v4 vault → user-login form
4. User selects account + enters password (+ MFA if required) → `unlock_vault_as_user`
5. Status badge: **unlocked** (green) → vault view  
...
| **v4 reload** | `reload_from_disk` keeps `current_user.dek` — only payload re-decrypted; user list read from updated header |
...
| **Host manifest** | `browser-extension/host/com.oxidvault.app.json` *(generated, gitignored)* | Browser registration (path + `allowed_origins`) |
| **Store extension ID** | `browser-extension/chrome-store-extension.id` | Single source for MSI WiX + `allowed_origins` store entry |
...
**Bridge state:** `AppState.bridge` (`BridgeAuthState`) remembers `mfa_unlock_failed` after `InvalidMfaCode` in `unlock_vault_as_user` — returned by `vault_status` / `get_login` as `mfa_failed`.
...
| **Rate limit** | Token bucket — max 10 `get_login` requests/minute per **peer IP** (not per connection; each request opens a fresh TCP connection with a new ephemeral port) → `{ "status": "error", "error": "rate_limited" }`. Token is validated **before** the rate limiter, so unauthenticated peers cannot consume tokens or trigger `BridgeThrottled` |
...
File: `browser-extension/host/com.oxidvault.app.json` *(generated by `register_native_host.ps1` or MSI; not committed)*

Template: `browser-extension/host/com.oxidvault.app.json.example`
...
```json
{
  ...
  "type": "stdio",
  "allowed_origins": ["chrome-extension://<store-extension-id>/"]
}
```
...
| `allowed_origins` | **Store ID** from `browser-extension/chrome-store-extension.id` (MSI + dev script); dev script may add a second entry for unpacked `extension.id` |
...
| `allowed_origins` | Store ID from `chrome-store-extension.id`; plus dev ID from `extension.id` / `-ExtensionId` when different |
...
| **Extension ID** | Store: `browser-extension/chrome-store-extension.id` (committed). Dev unpacked: `browser-extension/extension.id` (gitignored) |
...
| **Native messaging** | **Production:** MSI runs `InstallNativeMessagingHost` (WiX fragment in `native_messaging.wxs` + `componentRefs` in `tauri.conf.json` — required for Tauri to link the fragment). **Dev/debug:** `register_native_host.ps1` |
...
**Pre-checkpoint logs:** Audit logs without HMAC checkpoint lines report `auditChainAuthenticated: false` with status `audit_no_checkpoints` until the next unlocked session appends checkpoints.
...
|---|---|---|
| `VaultCreated` | `Vault::create_v3` | `-` |
| `VaultOpened` | `Vault::unlock_as_user` (success) | `-` |
...
| `VaultKeyRotated` | *(legacy audit log entries only)* | `-` |
| `UserPasswordChanged { username }` | `Vault::change_own_password` / `reencrypt_vault` | Username |
| `AuthFailed` | Failed unlock (wrong user password or MFA code on `unlock_vault_as_user`; legacy format on attach) | `-` |
...
| **Integration** | `Vault::create_v3`, `Vault::attach_locked` normalize before FS access |
...
> **Status:** ✅ `crates/vault-core/src/lock.rs` · `Vault::attach_locked()` · `Vault::close()` · `Drop`
...
| **Create** | `Vault::create_v3` acquires the lock for the whole session (released on write failure) |
| **Persist guard** | Every `persist()` asserts the held lock (`assert_lock_valid`) before writing |
...
```
Vault::attach_locked(path) / Vault::create_v3(path)
      │
      ▼
...
| Type | Role |
|---|---|
| **`AdminPolicy`** | Only set fields (`Option<T>`) are binding — incl. `kdfMemoryMib` (64–1024 MiB) |
| **`UserPolicyPreferences`** | Local values from `settings.json` + defaults |
| **`ResolvedConfig`** | Effective configuration after merge — incl. `kdfMemoryMib: ResolvedU32Field` (default 128 MiB) |
...
| `minMasterPasswordLen` | User password validation on **all** password-set paths: `create_vault_v3`, `add_vault_user`, `change_user_password` (`policy::effective_min_master_password_len`) |
| `kdfMemoryMib` | Argon2id memory (MiB, clamped 64–1024) for **new** KDF params on password-set paths via `policy::effective_kdf_params_for_new_vaults()`; exposed in `ResolvedConfig.kdfMemoryMib` (default 128 when unset) |
...
> **Status:** ✅ v4 per-user password rotation · `Vault::change_own_password` · `reencrypt_vault` alias · `compliance.rs` · `src/dashboard/ComplianceDashboard.tsx`

### Per-User Password Rotation (v4)

| Aspect | Detail |
|---|---|
| **Primary API** | `Vault::change_own_password(current, new)` — rewraps shared DEK with new user KEK, updates `password_changed_at`, **full payload re-encryption** (header AAD binding) |
| **Tauri commands** | `change_user_password` (explicit) · `reencrypt_vault` (compliance UI alias — delegates to `change_own_password`) |
| **Principle** | User password → KEK (Argon2id, effective KDF from policy) · shared DEK encrypts payload · DEK wrapped per user in `users_json` |
| **Rotation** | New per-user salt + new KEK; DEK unchanged; header + payload rewritten atomically |
| **Header fields** | `key_rotated_at` updated on successful rotation |
| **Audit** | Event `AuditAction::UserPasswordChanged { username }` |
| **UI** | `RotationDialog.tsx` — spinner during rotation, success updates compliance dashboard |

```
Current password → KEK_old → unwrap(DEK)
New password     → KEK_new → rewrap(DEK) → updated users_json
Payload          → decrypt + re-encrypt with fresh nonce + header AAD
```
...
| **Key age** | Days since `key_rotated_at` (fallback: `key_created_at`) |
| **Hint** | When key age > **90 days**: primary button **Rotate password** → `RotationDialog`; toast + refetch after `reencrypt_vault` / `change_user_password` |
| **Compliance badge** | "Compliance OK" (green) when audit chain valid, HMAC checkpoints OK (when applicable), and key age ≤ 90 days |
| **Rotation hint** | Below button: per-user KEK rewrap; payload re-encrypted with header AAD |
